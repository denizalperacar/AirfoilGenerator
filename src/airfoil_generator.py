...
class Airfoil:
  ...
  def write(self, file_name:str, update_name=True, save_pickle:bool=True, plot:bool=True) -> bool:
    ...
    try:
      ...
      with open(directory + f'/airofil/{self.name}.af', 'w') as fid:
        fid.write('index {}\n'.format(self.name))
  ...
  def _get_class(sampling_points:np.ndarray, N1:float, N2:float):
...
class AirfoilGenerator:
  ...
  def __init__(self, num_airfoils:int=20):
    self.start_index = self._read_last_index()
    self.num_airfoils = num_airfoils
    self.airfoils = []
    self.generate()

  def _read_last_index(self):
    if not os.path.exists('AirfoilGenerator/cfg/index.txt'):
      self.start_index = 0
    else:
      with open('AirfoilGenerator/cfg/index.txt', 'r') as fid:
  ...
  def generate(self) -> bool:
    ...
    try:
      ...
      for ind in range(
        ...
        directory = '{}/data/{}.af'.format(cd, ind)
        os.makedirs(directory, exist_ok = True)
        self.airfoils[-1].write(directory)
        
        with open('AirfoilGenerator/cfg/index.txt', 'w') as fid:
          fid.write(str(ind))
      success = True
    except:
      success = False
